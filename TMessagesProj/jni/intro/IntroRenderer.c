...
static TexturedShape telegram_sphere, telegram_plane, learn_shape, network_shape, secure_shape;
...
static GLuint learn_image, network_image, secure_image;
...
static inline float
calcParameterViaNewtonRaphsonUsingXAndCoefficientsForX(float x, float coefficientsX[]) {
    float t = x;
    int32_t i;
    for (i = 0; i < 10; i++) {
...
void setup_shaders() {
    ...
    color_program = get_color_program(
            build_program(vshader, (GLint) strlen(vshader), fshader, (GLint) strlen(fshader)));
    ...
    texture_program = get_texture_program(
            build_program(vshader_texture, (GLint) strlen(vshader_texture), fshader_texture,
                          (GLint) strlen(fshader_texture)));
    ...
    texture_program_blue = get_texture_program(
            build_program(vshader_texture_blue, (GLint) strlen(vshader_texture_blue),
                          fshader_texture_blue, (GLint) strlen(fshader_texture_blue)));
    ...
    texture_program_red = get_texture_program(
            build_program(vshader_texture_red, (GLint) strlen(vshader_texture_red),
                          fshader_texture_red, (GLint) strlen(fshader_texture_red)));
    ...
    texture_program_light_red = get_texture_program(
            build_program(vshader, (GLint) strlen(vshader), fshader, (GLint) strlen(fshader)));
    ...
    texture_program_light_blue = get_texture_program(
            build_program(vshader, (GLint) strlen(vshader), fshader, (GLint) strlen(fshader)));
    ...
    texture_program_one = get_texture_program(
            build_program(vshader, (GLint) strlen(vshader), fshader, (GLint) strlen(fshader)));
}
...
static inline void
mvp_matrix(mat4x4 model_view_projection_matrix, Params params, mat4x4 view_projection_matrix) {
    mat4x4 model_matrix;
    ...
    mat4x4_translate_independed(model_matrix, params.position.x, -params.position.y,
                                params.position.z);
    ...
    mat4x4_translate_independed(model_view_projection_matrix, 0,
                                -y_offset / view_projection_matrix[3][3], 0);
}

void draw_shape(const Shape *shape, mat4x4 view_projection_matrix) {
    if (shape->params.alpha > 0 &&
        (fabs(shape->params.scale.x) > 0 && fabs(shape->params.scale.y) > 0 &&
         fabs(shape->params.scale.z) > 0)) {
        ...
        glUniformMatrix4fv(color_program.u_mvp_matrix_location, 1, GL_FALSE,
                           (GLfloat *) model_view_projection_matrix);
        ...
        glVertexAttribPointer(color_program.a_position_location, 2, GL_FLOAT, GL_FALSE,
                              sizeof(CPoint), &shape->data[0].x);
...
void draw_textured_shape(const TexturedShape *shape, mat4x4 view_projection_matrix,
                         texture_program_type program_type) {
    if (shape->params.alpha > 0 &&
        (fabs(shape->params.scale.x) > 0 && fabs(shape->params.scale.y) > 0 &&
         fabs(shape->params.scale.z) > 0)) {
        mat4x4 model_view_projection_matrix;
        ...
        if (shape->params.const_params.is_star == 1) {
            ...
            int32_t d = 160;
            if (fabs(p_window[0]) > d || p_window[1] > y_offset_absolute * 2 + d ||
                p_window[1] < y_offset_absolute * 2 - d) {
                return;
            }
        }
        if (program_type == RED) {
        ...
        glUniformMatrix4fv(texture_program_temp->u_mvp_matrix_location, 1, GL_FALSE,
                           (GLfloat *) model_view_projection_matrix);
        ...
        glVertexAttribPointer(texture_program_temp->a_position_location, 2, GL_FLOAT, GL_FALSE,
                              4 * sizeof(GL_FLOAT), BUFFER_OFFSET(0));
        glVertexAttribPointer(texture_program_temp->a_texture_coordinates_location, 2, GL_FLOAT,
...
static inline void
gen_rounded_rectangle(CPoint *out, CSize size, float radius, int32_t round_count) {
    int32_t offset = 0;
    ...
    for (i = (round_count + 2) * n; i <= round_count + 1 + (round_count + 1) * n; i++) {
        out[offset++] = CPointMake(size.width / 2 - radius + cosf(i * k) * radius,
                                   size.height / 2 - radius + sinf(i * k) * radius);
    }
    n++;
...
Shape create_rounded_rectangle(CSize size, float radius, int32_t round_count, const vec4 color) {
    ...
    gen_rounded_rectangle(data, params.var_params.size, params.var_params.radius,
                          params.const_params.round_count);
    return (Shape) {{color[0], color[1], color[2], color[3]}, data,
...
void change_rounded_rectangle(Shape *shape, CSize size, float radius) {
    if ((*shape).params.var_params.size.width != size.width ||
        ...
        (*shape).params.var_params.size.height = size.height;
        (*shape).params.var_params.radius = radius;
        gen_rounded_rectangle((*shape).data, (*shape).params.var_params.size,
                              (*shape).params.var_params.radius,
                              (*shape).params.const_params.round_count);
        glBindBuffer(GL_ARRAY_BUFFER, shape->buffer);
...
static inline void
gen_segmented_square(CPoint *out, float side_length, float start_angle, float end_angle) {
    CPoint p;
...
TexturedShape
create_segmented_square(float side_length, float start_angle, float end_angle, GLuint texture) {
    ...
    return (TexturedShape) {texture, data,
                            create_vbo(params.const_params.datasize, data, GL_DYNAMIC_DRAW),
                            real_vertex_count, params};
}

void change_segmented_square(TexturedShape *shape, float side_length, float start_angle,
                             float end_angle) {
...
static inline void gen_circle(CPoint *out, float radius, int32_t vertex_count) {
    ...
    for (i = 0; i <= vertex_count; i++) {
        out[offset++] = CPointMake(radius * (cosf(2 * (float) M_PI * (i / (float) vertex_count))),
                                   radius * sinf(2 * (float) M_PI * (i / (float) vertex_count)));
    }
}
...
static inline void gen_infinity(CPoint *out, float width, float angle, int32_t segment_count) {
    ...
    for (seg = 0; seg <= segment_count; seg++) {
        ...
        vec2 p = {a.x * nt * nt * nt + 3.0f * p1.x * nt * nt * t + 3.0f * p2.x * nt * t * t +
                  b.x * t * t * t,
                  a.y * nt * nt * nt + 3.0f * p1.y * nt * nt * t + 3.0f * p2.y * nt * t * t +
                  b.y * t * t * t};
        vec2 tangent = {-3.0f * a.x * nt * nt + 3.0f * p1.x * (1.0f - 4.0f * t + 3.0f * t * t) +
                        3.0f * p2.x * (2.0f * t - 3.0f * t * t) + 3.0f * b.x * t * t,
                        -3.0f * a.y * nt * nt + 3.0f * p1.y * (1.0f - 4.0f * t + 3.0f * t * t) +
                        3.0f * p2.y * (2.0f * t - 3.0f * t * t) + 3.0f * b.y * t * t};
...
void change_infinity(Shape *shape, float angle) {
    if ((*shape).params.var_params.angle != angle) {
        ...
        gen_infinity(shape->data, (*shape).params.var_params.width,
                     (*shape).params.var_params.angle, (*shape).params.const_params.round_count);
        glBindBuffer(GL_ARRAY_BUFFER, shape->buffer);
        glBufferData(GL_ARRAY_BUFFER, shape->params.const_params.datasize, shape->data,
                     GL_DYNAMIC_DRAW);
        glBindBuffer(GL_ARRAY_BUFFER, 0);
    }
}
...
static inline void
gen_rounded_rectangle_stroked(CPoint *out, CSize size, float radius, float stroke_width,
                              int32_t round_count) {
    int32_t offset = 0;
    float k = (float) (M_PI / 2 / (round_count + 1));
    ...
    for (i = (round_count + 2) * n; i <= round_count + 1 + (round_count + 1) * n; i++) {
        out[offset++] = CPointMake(size.width / 2 - radius + cosf(i * k) * radius,
                                   size.height / 2 - radius + sinf(i * k) * radius);
        out[offset++] = CPointMake(size.width / 2 - radius + cosf(i * k) * inner_radius,
                                   size.height / 2 - radius + sinf(i * k) * inner_radius);
    }
    n++;
    for (i = (round_count + 1) * n; i <= round_count + 1 + (round_count + 1) * n; i++) {
        out[offset++] = CPointMake(-size.width / 2 + radius + cosf(i * k) * radius,
...
Shape
create_rounded_rectangle_stroked(CSize size, float radius, float stroke_width, int32_t round_count,
                                 const vec4 color) {
    ...
    CPoint *data = (CPoint *) malloc((size_t) params.const_params.datasize);
    gen_rounded_rectangle_stroked(data, params.var_params.size, params.var_params.radius,
                                  params.var_params.width, params.const_params.round_count);
    params.const_params.triangle_mode = GL_TRIANGLE_STRIP;
    return (Shape) {{color[0], color[1], color[2], color[3]}, data,
                    create_vbo(params.const_params.datasize, data, GL_DYNAMIC_DRAW),
                    real_vertex_count, params};
...
void change_rounded_rectangle_stroked(Shape *shape, CSize size, float radius) {
    if ((*shape).params.var_params.size.width != size.width ||
        ...
        (*shape).params.var_params.size.height = size.height;
        (*shape).params.var_params.radius = radius;
        gen_rounded_rectangle_stroked((*shape).data, (*shape).params.var_params.size,
                                      (*shape).params.var_params.radius,
                                      (*shape).params.var_params.width,
                                      (*shape).params.const_params.round_count);
        glBindBuffer(GL_ARRAY_BUFFER, shape->buffer);
        glBufferSubData(GL_ARRAY_BUFFER, 0, shape->params.const_params.datasize, shape->data);
        glBindBuffer(GL_ARRAY_BUFFER, 0);
...
float t(float start_value, float end_value, float start_time, float duration, timing_type type) {
    ...
    if (type == Linear) {
        return start_value + (end_value - start_value) *
                             MINf(duration + start_time, MAXf(.0, (time - start_time))) / duration;
    }
    return start_value + (end_value - start_value) *
                         timing(MINf(duration + start_time, MAXf(.0, (time - start_time))) /
                                duration, type);
}

float
t_reversed(float end_value, float start_value, float start_time, float duration, timing_type type) {
    if (time > start_time + duration) {
        return end_value;
    }
    if (type == Linear) {
        return start_value + (end_value - start_value) *
                             MINf(duration + start_time, MAXf(0.0f, (time - start_time))) /
                             duration;
    }
    return start_value + (end_value - start_value) *
                         timing(MINf(duration + start_time, MAXf(0.0f, (time - start_time))) /
                                duration, type);
}

float
t_local(float start_value, float end_value, float start_time, float duration, timing_type type) {
    if (type == Sin) {
        return start_value + (end_value - start_value) *
                             sinf(MINf(MAXf((time_local - start_time) / duration * (float) M_PI, 0),
                                       (float) M_PI));
    }
    ...
    if (type == Linear) {
        return start_value + (end_value - start_value) *
                             MINf(duration + start_time, MAXf(.0, (time_local - start_time))) /
                             duration;
    }
    return start_value + (end_value - start_value) *
                         timing(MINf(duration + start_time, MAXf(.0, (time_local - start_time))) /
                                duration, type);
}
...
static inline void
mat4x4_stars(mat4x4 m, float y_fov_in_degrees, float aspect, float n, float f, int32_t width,
             int32_t height) {
    if (height >= width) {
        float k = (float) width / (float) height;
        float q = 1.4f;
    ...
    mat4x4_translate_independed(m, 0, -2 * y_offset_absolute / (float) height +
                                      4 * scale_factor / (float) height, 0);
}
...
void mat4x4_layer(mat4x4 matrix, LayerParams params, float s, float r) {
    ...
    mat4x4_translate(model_matrix, -params.anchor.x / sc, params.anchor.y / sc * a,
                     params.anchor.z / sc);
    mat4x4 scaled;
    mat4x4_identity(scaled);
    float f = 1.0f;
    ...
    mat4x4_dup(tmp, model_matrix);
    mat4x4_mul(model_matrix, rotate, tmp);
    mat4x4_translate_independed(model_matrix, params.position.x / sc, -params.position.y / sc * a,
                                params.position.z / sc);
...
static void draw_ic(int32_t type) {
    ...
    if (type == 0) {
        ...
        if (current_page == 1 && direct == 0) {
            ...
            t_y = t(beginY, 0, commonDelay + duration_const * 0.2f * beginTimeK, duration_const,
                    EaseOut);
        ...
        float e = 2.71;
        float arg = MAXf(0, time - (commonDelay + duration_const * 0.2f * beginTimeK)) * 50;
        value = beginY * powf(e, -0.055f * arg * 2) * cosf(0.08f * arg - (float) M_PI / 8.0f) *
                0.4f;
        t_y -= value * bounce;
    }
    ...
    float pinasin = 0;
    ic_pin_layer.position = xyzMake(
            ic_pos.x + cosf(time_local * 5) * 3 * pinasin + cloud_scroll_offset,
            ic_pos.y + sinf(time_local * 5) * 1.5f * pinasin + t_y, 0);
    mat4x4_layer(ic_matrix, ic_pin_layer, 1, rotation);
    draw_textured_shape(&ic_pin, ic_matrix, COLOR);

    if (type == 1) {
        ic_videocam_layer.rotation = -30 + t_local(anim_videocam_old_angle, anim_videocam_angle,
                                                   anim_videocam_start_time, anim_videocam_duration,
                                                   EaseOut);
        t_y = t(beginY, 0, commonDelay + duration_const * 0.45f * beginTimeK, duration_const,
                EaseOut);
        float value = 0;
        float e = 2.71;
    ...
    if (type == 0) {
        ...
        if (current_page == 1 && direct == 0) {
            ...
            t_y = t(beginY, 0, commonDelay + duration_const * 0.3f * beginTimeK, duration_const,
                    EaseOut);
            float value = 0;
            float e = 2.71;
        ...
        t_y = t(beginY, 0, commonDelay + duration_const * 0.15f * beginTimeK, duration_const,
                EaseOut);
    ...
    ic_cam_layer.rotation = 15 + t_local(anim_cam_old_angle, anim_cam_angle, anim_cam_start_time,
                                         anim_cam_duration, EaseOut);
    ic_cam_layer.position = xyzMake(
            ic_pos.x + 0 *
                       t_local(anim_cam_old_position.x, anim_cam_position.x, anim_cam_start_time,
                               anim_cam_duration, EaseOut) + cloud_scroll_offset,
            ic_pos.y + 0 *
                       t_local(anim_cam_old_position.y, anim_cam_position.y, anim_cam_start_time,
                               anim_cam_duration, EaseOut)
            + t_y,
            0);
    ...
    if (type == 0) {
        ...
        if (current_page == 1 && direct == 0) {
            ...
            t_y = t(beginY, 0, commonDelay + duration_const * .0f * beginTimeK, duration_const,
                    EaseOut);
            float value = 0;
            float e = 2.71;
        ...
        t_y = t(beginY, 0, commonDelay + duration_const * 0.25f * beginTimeK, duration_const,
                EaseOut);
    ...
    if (type == 0) {
        ...
        if (current_page == 1 && direct == 0) {
            ...
            t_y = t(beginY, 0, commonDelay + duration_const * .45f * beginTimeK, duration_const,
                    EaseOut);
    ...
    if (type == 0) {
        ...
        if (current_page == 1 && direct == 0) {
            ...
            t_y = t(beginY, 0, commonDelay + duration_const * .1f * beginTimeK, duration_const,
                    EaseOut);
            float value = 0;
            float e = 2.71;
        ...
        t_y = t(beginY, 0, commonDelay + duration_const * 0.35f * beginTimeK, duration_const,
                EaseOut);
    ...
    if (anim_pencil_stage == 0) {
        ic_pencil_layer.rotation = t_local(0, -5, anim_pencil_start_all_time, duration_const * 0.5f,
                                           EaseOut);
        ...
        ic_pencil_layer.rotation = t_local(-5, 0, anim_pencil_start_all_end_time,
                                           duration_const * 0.5f, EaseOut);
    ...
    ic_pencil_layer.position = xyzMake(pencil_x + ic_pos.x + cloud_scroll_offset,
                                       pencil_v + ic_pos.y + t_y, 0);
...
JNIEXPORT void Java_org_telegram_messenger_Intro_onDrawFrame(JNIEnv *env, jclass class) {
    ...
    if (current_page == 0) {
        ...
        rglNormalDraw();
        learn_shape.params.alpha = 1;
        scale = 0.75;
        float alpha = 1;
        learn_shape.params.alpha = alpha;
        learn_shape.params.scale = xyzMake(scale, scale, 1);
    } else if (current_page == 2) {
        rglNormalDraw();
        network_shape.params.alpha = 1;
        scale = 0.75;
        float alpha = 1;
        network_shape.params.alpha = alpha;
        network_shape.params.scale = xyzMake(scale, scale, 1);
    } else if (current_page == 3) {
        rglNormalDraw();
        secure_shape.params.alpha = 1;
        scale = 0.75;
        float alpha = 1;
        secure_shape.params.alpha = alpha;
        secure_shape.params.scale = xyzMake(scale, scale, 1);
    } else if (current_page == 4) {
        if (direct == 1) {
            privateLayer.rotation = private_scroll_offset + t(-90, 0, 0, duration_const, EaseOut);
        } else {
            privateLayer.rotation =
                    private_scroll_offset + t(90, 0, 0, duration_const * private_back_k, EaseOut);
        }
    ...
    if (current_page == 0) {
        ...
        if (direct == 1) {
            ...
            mask1.params.rotation =
                    t(180, 0.0f, 0, duration_const, EaseInEaseOut) + free_scroll_offset;
        }
    } else if (current_page == 2) {
        if (direct == 1) {
            ...
            mask1.params.rotation =
                    t(0, 180.0f, 0, duration_const, EaseInEaseOut) + free_scroll_offset;
        } else {
            free_scroll_offset = scroll_offset * 5;
            float r = 316 / 4.0f;
            ...
            change_rounded_rectangle(&mask1, CSizeMake(size, size), round);
            mask1.params.rotation = t_reversed(180.0f + free_scroll_offset, 180.0f + 90.0f, 0,
                                               duration_const, EaseInEaseOut);
        }
    } else if (current_page == 3) {
        if (direct == 1) {
            float r = 316 / 4.0f;
            ...
            change_rounded_rectangle(&mask1, CSizeMake(size, size), round);
            mask1.params.rotation = t(180.0f + free_scroll_offset, 180.0f + 90.0f, 0,
                                      duration_const, EaseInEaseOut);
        } else {
            float r = 316 / 4.0f;
            ...
            change_rounded_rectangle(&mask1, CSizeMake(size, size), round);
            mask1.params.rotation = t_reversed(180.0f + 90.0f, 180.0f + 90.0f + 90.0f, 0,
                                               duration_const, EaseOut);
        ...
        if (direct == 1) {
            ...
            change_rounded_rectangle(&mask1, CSizeMake(size, size), round);
            mask1.params.rotation = private_scroll_offset +
                                    t(180.0f + 90.0f, 180.0f + 90.0f + 90.0f, 0, duration_const,
                                      EaseOut);
            mask1.params.position = xyzMake(0, 0, mask1.params.position.z);
        } else {
            float k = 0;
            ...
            change_rounded_rectangle(&mask1, CSizeMake(scale, scale),
                                     t_reversed(30, 50, 0, duration_const * k, EaseOut));
            mask1.params.position = xyzMake(t_reversed(0, 29 / 2, 0, duration_const * k, EaseOut),
                                            t_reversed(0, -19 / 2, 0, duration_const * k, EaseOut),
                                            mask1.params.position.z);
            mask1.params.rotation = private_scroll_offset + t_reversed(180.0f + 90.0f + 90.0f,
                                                                       180.0f + 90.0f + 90.0f +
                                                                       90.0f, 0, duration_const * k,
                                                                       EaseOut);

            k = 1.0f * private_back_k;
            int32_t sublayer2_radius = 33;
            cloud_extra_mask1.params.position = xyzMake(
                    t_reversed(0, -122 / 2, 0, duration_const * k, EaseOut),
                    t_reversed(0, 54 / 2 - 1, 0, duration_const * k, EaseOut),
                    cloud_extra_mask1.params.position.z);
            ...
            cloud_extra_mask2.params.position = xyzMake(
                    t_reversed(0, -84 / 2, 0, duration_const * k, EaseOut),
                    t_reversed(0, -29 / 2, 0, duration_const * k, EaseOut),
                    cloud_extra_mask2.params.position.z);
            ...
            cloud_extra_mask3.params.position = xyzMake(
                    t_reversed(0, 128 / 2, 0, duration_const * k, EaseOut),
                    t_reversed(0, 56 / 2, 0, duration_const * k, EaseOut),
                    cloud_extra_mask3.params.position.z);
            ...
            cloud_extra_mask4.params.position = xyzMake(
                    t_reversed(0, 0, 0, duration_const * k, EaseOut),
                    t_reversed(0, 50, 0, duration_const * k, EaseOut),
                    cloud_extra_mask4.params.position.z);
        ...
        change_rounded_rectangle(&mask1, CSizeMake(scale, scale),
                                 t(30, 50, 0, duration_const * k, EaseOut));
        mask1.params.position = xyzMake(t(0, 29 / 2, 0, duration_const * k, EaseOut),
                                        t(0, -19 / 2, 0, duration_const * k, EaseOut),
                                        mask1.params.position.z);
        mask1.params.rotation = t(180.0f + 90.0f + 90.0f, 180.0f + 90.0f + 90.0f + 90.0f, 0,
                                  duration_const * k, EaseOut);

        k = 1.0f;
        int32_t sublayer2_radius = 33;
        cloud_extra_mask1.params.position = xyzMake(t(0, -122 / 2, 0, duration_const * k, EaseOut),
                                                    t(0, 54 / 2 - 1, 0, duration_const * k,
                                                      EaseOut),
                                                    cloud_extra_mask1.params.position.z);
        ...
        k = 1.15;
        int32_t sublayer3_radius = 94 / 4;
        cloud_extra_mask2.params.position = xyzMake(t(0, -84 / 2, 0, duration_const * k, EaseOut),
                                                    t(0, -29 / 2, 0, duration_const * k, EaseOut),
                                                    cloud_extra_mask2.params.position.z);
        ...
        k = 1.3;
        int32_t sublayer4_radius = 124 / 4;
        cloud_extra_mask3.params.position = xyzMake(t(0, 128 / 2, 0, duration_const * k, EaseOut),
                                                    t(0, 56 / 2, 0, duration_const * k, EaseOut),
                                                    cloud_extra_mask3.params.position.z);
        ...
        k = 1.5f;
        int32_t sublayer5_radius = 64;
        cloud_extra_mask4.params.position = xyzMake(t(0, 0, 0, duration_const * k, EaseOut),
                                                    t(0, 50, 0, duration_const * k, EaseOut),
                                                    cloud_extra_mask4.params.position.z);
    ...
    if (current_page == 0) {
        ...
        rglNormalDraw();
        learn_shape.params.alpha = t(0, 1, 0, duration_const * 0.8f, Linear);
        scale = 1;

        learn_shape.params.scale = xyzMake(scale, scale, 1);
        draw_textured_shape(&learn_shape, main_matrix, NORMAL);
        ...
        rglNormalDraw();
        network_shape.params.alpha = t(0, 1, 0, duration_const * 0.8f, Linear);
        scale = 1;

        network_shape.params.scale = xyzMake(scale, scale, 1);
        draw_textured_shape(&network_shape, main_matrix, NORMAL);
        ...
        rglNormalDraw();
        secure_shape.params.alpha = t(0, 1, 0, duration_const * 0.8f, Linear);
        scale = 1;

        secure_shape.params.scale = xyzMake(scale, scale, 1);
        draw_textured_shape(&secure_shape, main_matrix, NORMAL);
...
JNIEXPORT void
Java_org_telegram_messenger_Intro_setScrollOffset(JNIEnv *env, jclass class, float a_offset) {
...
JNIEXPORT void
Java_org_telegram_messenger_Intro_setIcTextures(JNIEnv *env, jclass class, GLuint a_ic_bubble_dot,
                                                GLuint a_ic_bubble, GLuint a_ic_cam_lens,
                                                GLuint a_ic_cam, GLuint a_ic_pencil,
                                                GLuint a_ic_pin, GLuint a_ic_smile_eye,
                                                GLuint a_ic_smile, GLuint a_ic_videocam) {
...
JNIEXPORT void Java_org_telegram_messenger_Intro_setTelegramTextures(JNIEnv *env, jclass class,
                                                                     GLuint a_telegram_sphere) {
    telegram_sphere_texture = a_telegram_sphere;
//    telegram_plane_texture = a_telegram_plane;
}

JNIEXPORT void
Java_org_telegram_messenger_Intro_setLearnTextures(JNIEnv *env, jclass class, GLuint a_learn_iage) {
    learn_image = a_learn_iage;
}

JNIEXPORT void Java_org_telegram_messenger_Intro_setGrowNetworkTextures(JNIEnv *env, jclass class,
                                                                        GLuint a_network_image) {
    network_image = a_network_image;
}

JNIEXPORT void Java_org_telegram_messenger_Intro_setSecureTextures(JNIEnv *env, jclass class,
                                                                   GLuint a_secure_image) {
    secure_image = a_secure_image;
}
...
//    telegram_plane.params.anchor = xyzMake(6, -5, 0);
    ...
    learn_shape = create_textured_rectangle(CSizeMake(150, 150), learn_image);
    network_shape = create_textured_rectangle(CSizeMake(150, 150), network_image);
    secure_shape = create_textured_rectangle(CSizeMake(150, 150), secure_image);
...
JNIEXPORT void
Java_org_telegram_messenger_Intro_onSurfaceChanged(JNIEnv *env, jclass class, int32_t a_width_px,
                                                   int32_t a_height_px, float a_scale_factor,
                                                   int32_t a1) {
    ...
    mat4x4_plain(main_matrix, (int32_t) ((float) a_width_px / a_scale_factor),
                 (int32_t) ((float) a_height_px / a_scale_factor));
    offset_y = a1 * main_matrix[1][1];
    set_y_offset_objects(offset_y);
    y_offset_absolute = a1;
    mat4x4_stars(stars_matrix, 45, 1, -1000, 0, (int32_t) ((float) a_width_px / a_scale_factor),
                 (int32_t) ((float) a_height_px / a_scale_factor));
}