...
import org.flinkhub.ui.PopupNotificationActivity;
...
public class NotificationsController extends BaseController {
    ...
    public void processNewMessages(final ArrayList<MessageObject> messageObjects, final boolean isLast, final boolean isFcm, CountDownLatch countDownLatch) {
        ...
        notificationsQueue.postRunnable(() -> {
            ...
            for (int a = 0; a < messageObjects.size(); a++) {
                ...
                // Flinkhub: Hiding notifications of other users joining groups
                if (
                        messageObject.messageOwner != null &&
                                (
                                        messageObject.messageOwner.action instanceof TLRPC.TL_messageActionChatJoinedByLink ||
                                                messageObject.messageOwner.action instanceof TLRPC.TL_messageActionChatAddUser ||
                                                messageObject.messageOwner.action instanceof TLRPC.TL_messageActionChatDeleteUser
                                )
                ) {
                    continue;
                }

                // Flinkhub: Hiding notifications of contact joined telegram
                if (
                        messageObject.messageOwner != null &&
                                (
                                        messageObject.messageOwner.action instanceof TLRPC.TL_messageActionUserJoined ||
                                                messageObject.messageOwner.action instanceof TLRPC.TL_messageActionContactSignUp
                                )
                ) {
                    continue;
                }

                if (AndroidUtilities.isTelegramInstalled()) {
                    continue;
                }
    ...
    public void deleteNotificationChannel(long dialogId) {
        notificationsQueue.postRunnable(() -> {
            ...
            try {
                SharedPreferences preferences = getAccountInstance().getNotificationsSettings();
                String key = "org.flinkhub.key" + dialogId;
        ...
        notificationsQueue.postRunnable(() -> {
            ...
            try {
                ...
                for (Map.Entry<String, ?> entry : values.entrySet()) {
                    ...
                    if (key.startsWith("org.flinkhub.key")) {
                        if (!key.endsWith("_s")) {
    ...
    private void showExtraNotifications(NotificationCompat.Builder notificationBuilder, boolean notifyAboutLast, String summary) {
        ...
        for (int b = 0, size = sortedDialogs.size(); b < size; b++) {
            ...
            msgHeardIntent.setAction("org.flinkhub.messenger2.ACTION_MESSAGE_HEARD");
